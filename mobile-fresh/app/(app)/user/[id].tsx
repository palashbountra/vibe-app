...
import { MOCK_FEED, matchingService } from '@/services/matchingService';
...
export default function UserProfileScreen() {
  ...
  const candidate = MOCK_FEED.find((c) => c.id === id);
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <Text style={{ color: Colors.textSecondary, ...Typography.body }}>Profile not found.</Text>
        </View>
      </SafeAreaView>
    );
  }

  const hasPhotos = candidate.photos && candidate.photos.length > 0;
  const photoCount = hasPhotos ? candidate.photos!.length : 1;

  const handleConnect = async () => {
    if (connected) return;
    await matchingService.sendConnectionRequest(candidate.id);
    setConnected(true);
  };
  ...
  const sendCompliment = async () => {
    ...
    setConnected(true);
    Alert.alert('Sent!', `Your compliment was sent to ${candidate.displayName} along with a connection request.`);
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} bounces={true}>
        ...
        <View style={styles.heroContainer}>
          {hasPhotos ? (
            ...
            <View style={[styles.avatarHero, { backgroundColor: candidate.avatarColor }]}>
          ...
          <View style={[styles.compatOverlay, { backgroundColor: compatColor(candidate.compatibilityScore) }]}>
            <Ionicons name="musical-notes" size={12} color="#FFFFFF" />
            <Text style={styles.compatOverlayText}>{candidate.compatibilityScore}% match</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.nameRow}>
            <Text style={styles.nameText}>{candidate.displayName}</Text>
            {candidate.age && <Text style={styles.ageText}>,{' '}{candidate.age}</Text>}
          </View>
          ...
          <View style={styles.metaRow}>
            {candidate.pronouns && (
              <View style={styles.metaChip}>
                <Text style={styles.metaChipText}>{candidate.pronouns}</Text>
              </View>
            )}
            {candidate.location && (
              <View style={styles.metaChip}>
                <Ionicons name="location-outline" size={12} color={Colors.textSecondary} />
                <Text style={styles.metaChipText}>{candidate.location}</Text>
              </View>
            )}
          </View>

          {(candidate.job || candidate.school) && (
            <View style={styles.infoRow}>
              {candidate.job && (
                <View style={styles.infoItem}>
                  <Ionicons name="briefcase-outline" size={14} color={Colors.textSecondary} />
                  <Text style={styles.infoText}>{candidate.job}</Text>
                </View>
              )}
              {candidate.school && (
                <View style={styles.infoItem}>
                  <Ionicons name="school-outline" size={14} color={Colors.textSecondary} />
                  <Text style={styles.infoText}>{candidate.school}</Text>
                </View>
              )}
              {candidate.height && (
                <View style={styles.infoItem}>
                  <Ionicons name="resize-outline" size={14} color={Colors.textSecondary} />
                  <Text style={styles.infoText}>{candidate.height}</Text>
                </View>
              )}
            </View>
          )}
          ...
          <View style={styles.nowPlayingCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.nowPlayingTrack} numberOfLines={1}>
                {candidate.currentTrack.title}
              </Text>
              <Text style={styles.nowPlayingArtist} numberOfLines={1}>
                {candidate.currentTrack.artist}
              </Text>
            </View>
        ...
        {candidate.prompts && candidate.prompts.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>About {candidate.displayName}</Text>
            {candidate.prompts.map((prompt, i) => (
        ...
        <View style={styles.section}>
          ...
          <View style={styles.artistList}>
            {candidate.topArtists.map((artist, i) => (
              <View key={artist} style={styles.artistRow}>
                <View style={[styles.artistRank, { backgroundColor: candidate.avatarColor + '33' }]}>
                  <Text style={[styles.artistRankText, { color: candidate.avatarColor }]}>{i + 1}</Text>
                </View>
          ...
          <View style={styles.compatBreakdown}>
            <Text style={[styles.compatBreakdownLabel, { color: Colors.textSecondary }]}>Compatibility breakdown</Text>
            {[
              { label: 'Genres', value: candidate.compatibilityBreakdown.genre },
              { label: 'Artists', value: candidate.compatibilityBreakdown.artist },
              { label: 'Vibe', value: candidate.compatibilityBreakdown.audio },
            ].map(({ label, value }) => (
              <View key={label} style={styles.breakdownRow}>
                <Text style={styles.breakdownLabel}>{label}</Text>
                <View style={styles.breakdownBarBg}>
                  <View style={[styles.breakdownBar, { width: `${Math.round(value * 100)}%` as any, backgroundColor: compatColor(Math.round(value * 100)) }]} />
                </View>
      ...
      <View style={styles.actionBar}>
        ...
        >
          ...
          <Text style={styles.connectBtnText}>
            {connected ? 'Request Sent' : `Connect with ${candidate.displayName}`}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={styles.modalSheet}>
          ...
          <TextInput
            ...
            placeholder={`Say something to ${candidate.displayName}...`}
          ...
          <TouchableOpacity
            style={[
              styles.sendBtn,
              (!complimentText.trim() || sending) && styles.sendBtnDisabled,
            ]}
          ...
          <TouchableOpacity style={styles.skipBtn} onPress={() => { setComplimentModal(false); handleConnect(); }}>
